...
TEST_DOCKER_REPO=nephirim/incubusdnode
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=go-incubus \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=incubusd \
...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build -mod=readonly $(BUILD_FLAGS) -o build/incubusd.exe ./cmd/incubusd
else
	go build -mod=readonly $(BUILD_FLAGS) -o build/incubusd ./cmd/incubusd
endif
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/incubusd
...
build-docker-go-incubus:
	$(MAKE) -C contrib/localnet

# Run a 4-node testnet locally
localnet-start: build-linux build-docker-incubusdnode
	@if ! [ -f build/node0/incubusd/config/genesis.json ]; \
		then docker run --rm -v $(CURDIR)/build:/incubusd:Z nephirim/incubusdnode testnet --v 4 -o . --starting-ip-address 192.168.10.2 --keyring-backend=test ; \
	fi
	docker-compose up -d
...
draw-deps:
	...
	@goviz -i ./cmd/incubusd -d 2 | dot -Tpng -o dependency-graph.png
...
.PHONY: all build-linux install install-debug \
	...
	build-docker-incubusdnode localnet-start localnet-stop test-docker test-docker-push \
